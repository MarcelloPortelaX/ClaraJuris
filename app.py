...
def normalize_database_url(raw_url: str | None) -> str:
    """Normaliza a URL do banco para produção no Render/Supabase/Neon.

    - Sem DATABASE_URL: usa SQLite local.
    - Com DATABASE_URL: usa PostgreSQL externo.
    - Se a senha tiver # sem codificação, converte para %23 antes do @.
    - Aceita postgres://, postgresql:// e postgresql+psycopg2://.
    """
    if not raw_url:
...
DB_IS_EXTERNAL = not DATABASE_URL.startswith("sqlite:///")
DB_BACKEND_LABEL = "Banco externo" if DB_IS_EXTERNAL else "Banco local"
DB_STORAGE_DESCRIPTION = (
    "Anotações e temas personalizados ficam salvos no banco de dados do app e são recarregados automaticamente."
)
DB_STORAGE_TECHNICAL_DESCRIPTION = (
    "Produção: PostgreSQL externo configurado pela variável DATABASE_URL."
    if DB_IS_EXTERNAL
    else "Modo local: fallback SQLite em data/anotacoes.db. No Render, configure DATABASE_URL para persistência real."
)
...
def compact_label(value: object, width: int = 54) -> str:
    """Cria rótulos curtos para gráficos sem perder o texto completo no hover."""
    text_value = str(value or "").strip()
    return textwrap.shorten(text_value, width=width, placeholder="...") if len(text_value) > width else text_value
    ...
    with engine.begin() as conn:
        conn.execute(
            text(
                ...
                VALUES (:materia, :tema, :titulo, :conteudo, :prioridade, :criado_em, :atualizado_em)
                """
            ),
            {
                "materia": materia,
...
def read_notes(limit: int = 40) -> pd.DataFrame:
    with engine.connect() as conn:
        return pd.read_sql_query(
            text("SELECT id, materia, tema, titulo, conteudo, prioridade, criado_em FROM anotacoes ORDER BY id DESC LIMIT :limit"),
            conn,
...
def count_notes() -> int:
    with engine.connect() as conn:
        row = conn.execute(text("SELECT COUNT(*) FROM anotacoes")).scalar()
        return int(row or 0)
    ...
    with engine.begin() as conn:
        conn.execute(
            ...
            ),
            {
                "materia": materia,
                "tema": tema.strip(),
                "incidencia_concurso": int(incidencia),
                "dificuldade": int(dificuldade),
                "prioridade_oab": int(prioridade_oab),
                "prioridade_concurso_publico": int(prioridade_concurso),
                "area_concurso": (area or "Geral").strip(),
                "descricao_curta": (descricao or "Tema adicionado manualmente.").strip(),
                "criado_em": now,
            },
        )
...
def read_custom_topics(include_meta: bool = False) -> pd.DataFrame:
    ...
    try:
        with engine.connect() as conn:
            df = pd.read_sql_query(text("SELECT * FROM temas_personalizados ORDER BY id DESC"), conn)
    except Exception:
...
def figure_top_topics_by_exam(df: pd.DataFrame, exam: str, theme: str | None = "light") -> go.Figure:
    exam_df = topics_for_exam(df, exam).sort_values("peso_concurso_x", ascending=True).tail(10).copy()
    exam_df["tema_grafico"] = exam_df["tema"].apply(lambda value: compact_label(value, width=58))
    exam_label = EXAM_RULES.get(exam, EXAM_RULES["GERAL"])["label"]
    fig = px.bar(
        exam_df,
        x="peso_concurso_x",
        ...
        hover_data={
            ...
            "materia": True,
            "dificuldade": True,
            "incidencia_concurso": True,
            "area_concurso": True,
            "peso_concurso_x": ":.1f",
        },
        title=f"Top temas para {exam_label}",
        labels={"peso_concurso_x": "Peso no concurso", "tema_grafico": ""},
    )
    fig = base_fig(fig, 620, theme)
    fig.update_layout(
        bargap=0.22,
        margin={"l": 230, "r": 28, "t": 68, "b": 112},
        legend={
            "orientation": "h",
            "yanchor": "top",
            "y": -0.20,
            ...
            "x": 0,
            "title_text": "Matéria",
            "font": {"color": theme_config(theme)["font"]},
        },
    )
    fig.update_xaxes(range=[0, 105], automargin=True)
...
def note_cards(df: pd.DataFrame) -> html.Div:
    if df.empty:
        return html.Div(
            className="note-card",
            children=[html.H3("Nenhuma anotação ainda"), html.P("Escreva uma nota de aula, dúvida ou resumo. O conteúdo será salvo no banco de dados do app.")],
        )
    cards = []
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.P(f"Armazenamento ativo: {DB_BACKEND_LABEL}.", className="storage-status"),
                    ],
                ),
            ],
        ),
    ...
    if tab == "concursos":
        ...
        return html.Div(
            children=[
                ...
                html.Div(
                    className="exam-chart-stack",
                    children=[
                        html.Div(className="panel chart-panel", children=[dcc.Graph(figure=figure_subjects_by_exam(topics, concurso_x, theme), config={"responsive": True})]),
                        html.Div(className="panel chart-panel chart-wide", children=[dcc.Graph(figure=figure_top_topics_by_exam(topics, concurso_x, theme), config={"responsive": True})]),
                    ],
                ),
                html.Div(
                    className="two-column",
                    children=[
                        html.Div(className="panel chart-panel", children=[dcc.Graph(figure=figure_exam_heatmap(topics, theme), config={"responsive": True})]),
                        html.Div(className="panel chart-panel", children=[dcc.Graph(figure=figure_difficulty(topics, theme), config={"responsive": True})]),
                    ],
                ),
                panel(
    ...
    if tab == "anotacoes":
        ...
        return html.Div(
            children=[
                ...
                panel("Anotações salvas", note_cards(notes), DB_STORAGE_DESCRIPTION),
            ]
        )
            ...
            children=[
                html.Div(
                    ...
                    children=[
                        ...
                        html.P("Complete a base com o que o professor passou, tópico de aula, lista de exercícios ou tema de prova."),
                        html.Div(
                            className="form-grid",
                            children=[
                                html.Div(
            ...
            children=[
                ...
                panel(
                    ...
                    html.Div(
                        ...
                        children=[
                            html.P(f"Backend ativo: {DB_BACKEND_LABEL}."),
                            html.P(DB_STORAGE_DESCRIPTION),
                            html.P(DB_STORAGE_TECHNICAL_DESCRIPTION),
                            html.P("Tudo que for criado pela interface — anotações e temas personalizados — fica salvo nesse banco e é recarregado nos gráficos, tabelas e rankings."),
                        ],
                    ),
                ),
                html.Div(
...
def save_custom_topic(n_clicks, materia, tema, incidencia, dificuldade, prioridade_oab, prioridade_concurso, area, descricao, refresh):
    ...
    try:
        ...
        return f"Não foi possível salvar no banco ativo: {exc}", refresh
    return "Tema salvo. Ele já entra nos gráficos, tabelas e rankings.", int(refresh or 0) + 1
